...
class AES ():
    ...
    def encrypt_x931(self, plaintext_bv, key_bv):
        BLOCKSIZE = 128
        ciphertext_bv = BitVector(size = 0)
    ...
    def ctr_aes_image(self , iv, image_file, enc_image):
        ...
        plaintext_bv = BitVector(filename = image_file)
        ciphertext_bv = BitVector(size = 0)
        ...
        numReturns = 0
        while(numReturns < 3):
            bv = plaintext_bv.read_bits_from_file(8)
            if (bv.int_val() == 10):
                numReturns += 1
                bv.write_to_file(outFile)
            else:
                bv.write_to_file(outFile)
        
        while (plaintext_bv.more_to_read):
            bv = plaintext_bv.read_bits_from_file(BLOCKSIZE)
            if len(bv) < BLOCKSIZE:
                temp = BitVector(intVal = 0, size = BLOCKSIZE-len(bv))
                bv = bv + temp
