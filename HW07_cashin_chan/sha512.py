...
# Some code taken from Lecture 15 SHA256.py file
class SHA512( object ):
    def __init__(self, message):
        self.bv = BitVector(textstring=message)

    def sha512(self):

        #  The 8 64-bit words used for initializing the 512-bit hash buffer before we start scanning the
        ...
        for n in range(0,bv4.length(), 1024):
            ...
            #                                   512-bit input message block.  There are a total of 80 
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        hasher = SHA512(message = input_hash)
        hash_hex_string = hasher.sha512()
